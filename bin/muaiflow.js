...
function init(force) {
  ...
  console.log(
    '  Next: run `npx muaiflow examples` to copy AGENTS.md / CLAUDE.md templates.'
  )
...
function createPlan(commandArgs) {
  ...
  const linkedDestPath = `@${relativeDestPath}`

  fs.mkdirSync(planDir, { recursive: true })

  if (fs.existsSync(destPath) && !force) {
    console.error('\n  Plan already exists: ' + relativeDestPath)
    console.error('  Use --force to overwrite.\n')
    process.exit(1)
  }

  fs.copyFileSync(templatePath, destPath)
  console.log('\n  Plan created: ' + linkedDestPath)
  console.log('  Next: copy paste this info above:')
  console.log('  Read @.ai/prompts/plan-generation.prompt.md')
  console.log('  Fill ' + linkedDestPath)
  console.log('  Analyze @.ai/plans/context.md')
  console.log('')
  console.log('  Copy/paste to reviewer AI:')
  console.log(
    '  Follow @.ai/prompts/multi-ai-review.prompt.md to validate ' +
      linkedDestPath +
      '\n'
  )
}
...
function createContext(commandArgs) {
  const reset =
    commandArgs.includes('--reset') || commandArgs.includes('--force')
  ...
  if (fs.existsSync(contextPath) && !reset) {
    ...
    console.log(
      '  Use `npx muaiflow context --reset` to reset it from CONTEXT_TEMPLATE.md.'
    )
