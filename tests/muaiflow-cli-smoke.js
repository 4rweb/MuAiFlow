...
function runNode(args, cwd, options = {}) {
  ...
  if (options.expectFailure) {
    assert.notStrictEqual(
      result.status,
      0,
      'command was expected to fail: node ' + args.join(' ')
    )
    return result
  }
...
try {
  ...
  assert(
    exists('.ai/plans/CONTEXT_TEMPLATE.md', tempRoot),
    'CONTEXT_TEMPLATE.md should exist'
  )
  assert(
    exists('.ai/plans/.gitignore', tempRoot),
    'plans/.gitignore should exist'
  )
  assert(
    exists('.ai/plans/context.md', tempRoot),
    'context.md should be created'
  )
  assert(
    exists('.ai/plans/tracked/.gitkeep', tempRoot),
    'tracked/.gitkeep should exist'
  )
  assert(
    exists('.ai/plans/local/.gitignore', tempRoot),
    'local/.gitignore should exist'
  )

  const date = todayPrefix()
  const template = read('.ai/plans/TEMPLATE.md', tempRoot)

  const trackedPlanResult = runNode(
    [cli, 'plan', 'my-feature', '--tracked'],
    tempRoot
  )
  const trackedPlan = `.ai/plans/tracked/${date}-my-feature.md`
  assert.strictEqual(
    read(trackedPlan, tempRoot),
    template,
    'tracked plan should be copied from TEMPLATE.md'
  )
  assert.match(
    trackedPlanResult.stdout,
    /Next: copy paste this info above:/,
    'tracked plan output should include next step header'
  )
  assert.match(
    trackedPlanResult.stdout,
    /Read @\.ai\/prompts\/plan-generation\.prompt\.md/,
    'tracked plan output should include the prompt path'
  )
  assert.match(
    trackedPlanResult.stdout,
    new RegExp(`Fill @${escapeRegExp(trackedPlan)}`),
    'tracked plan output should include the created plan path'
  )
  assert.match(
    trackedPlanResult.stdout,
    /Analyze @\.ai\/plans\/context\.md/,
    'tracked plan output should include the context path'
  )
  assert.match(
    trackedPlanResult.stdout,
    new RegExp(
      `Follow @\\.ai/prompts/multi-ai-review\\.prompt\\.md to validate @${escapeRegExp(trackedPlan)}`
    ),
    'tracked plan output should include the reviewer copy/paste line'
  )
  ...
  const localPlanResult = runNode(
    [cli, 'plan', 'my-feature', '--local'],
    tempRoot
  )
  const localPlan = `.ai/plans/local/${date}-my-feature.md`
  assert.strictEqual(
    read(localPlan, tempRoot),
    template,
    'local plan should be copied from TEMPLATE.md'
  )
  assert.match(
    localPlanResult.stdout,
    new RegExp(`Fill @${escapeRegExp(localPlan)}`),
    'local plan output should include the created plan path'
  )
  assert.match(
    localPlanResult.stdout,
    new RegExp(
      `Follow @\\.ai/prompts/multi-ai-review\\.prompt\\.md to validate @${escapeRegExp(localPlan)}`
    ),
    'local plan output should include the reviewer copy/paste line'
  )
  ...
  assert.strictEqual(
    ...
    'custom context',
    'context command should preserve existing context'
  )
  ...
  assert.strictEqual(
    read('.ai/plans/context.md', tempRoot),
    '',
    'context --clear should empty context.md'
  )
  ...
  runNode([cli, 'context', '--reset', '--clear'], tempRoot, {
    expectFailure: true
  })

  write('.ai/plans/context.md', tempRoot, 'preserve context')
  write(
    '.ai/plans/tracked/2099-01-01-user-plan.md',
    tempRoot,
    'preserve tracked'
  )
  ...
  assert.strictEqual(
    read('.ai/plans/context.md', tempRoot),
    'preserve context',
    'init --force should preserve context.md'
  )
  assert.strictEqual(
    read('.ai/plans/tracked/2099-01-01-user-plan.md', tempRoot),
    'preserve tracked',
    'init --force should preserve tracked plans'
  )
  assert.strictEqual(
    read('.ai/plans/local/2099-01-01-local-plan.md', tempRoot),
    'preserve local',
    'init --force should preserve local plans'
  )
  assert.strictEqual(
    read('.ai/plans/2099-01-01-legacy-plan.md', tempRoot),
    'preserve legacy',
    'init --force should preserve legacy top-level plans'
  )
  ...
  const handoff = runCommand(
    'bash',
    ['.ai/scripts/handoff.sh', 'codex'],
    tempRoot
  )
  assert(
  ...
  assert(
    handoff.stdout.includes('Status:  EXECUTING'),
    'handoff should parse status with portable awk'
  )

  postinstallRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), 'muaiflow-postinstall-')
  )
  runNode([postinstall], repoRoot, { env: { INIT_CWD: postinstallRoot } })
  assert(
    exists('.ai/plans/context.md', postinstallRoot),
    'postinstall should create context.md'
  )
  ...
  const gitVersion = runCommand('git', ['--version'], tempRoot, {
    optional: true
  })
  if (gitVersion) {
